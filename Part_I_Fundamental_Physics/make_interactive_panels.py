"""
Generate standalone interactive HTML panels for Part II physics outputs.
...
<body><header><h1>{title}</h1><p>Interactive local panel for figures and numerical output tables.</p><p class="links"><a href="../interactive_index.html">Physics interactive index</a></p></header>
...
def make_index() -> None:
    ...
    html = f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Part II Physics Interactive Panels</title><style>{STYLE}</style></head>
<body><header><h1>Part II Physics Interactive Panels</h1><p>Local HTML dashboards for the twelve physics papers.</p></header><main><section class="panel">{''.join(links)}</section><section class="note"><p>Panels preview existing figure files and render quick charts from CSV outputs. They are standalone and offline.</p></section></main></body></html>"""
